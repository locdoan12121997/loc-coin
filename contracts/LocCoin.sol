...
contract SafeMath {
    ...
    // @param a Variable 1
    // @param b Variable 2
    function pow( uint256 a , uint8 b ) internal pure returns ( uint256 ){
        uint256 c;
        c = a ** b;
        return c;
    }
...
contract ERC20Token is owned, SafeMath{
    ...
    string public name = "Rockship";
    string public symbol = "RKSH";
    uint256 public decimals = 18;
    uint256 public totalSupply = 24000000;
    address public ico;
...
contract DDDico is SafeMath {
    ...
    uint256 public block0 = now;
    uint256 public block1 = block0 + 2 minutes;
    uint256 public block2 = block0 + 4 minutes;
    uint256 public block3 = block0 + 6 minutes;
    uint256 public block4 = block0 + 8 minutes;
    uint256 public block5 = block0 + 10 minutes;
    uint256 public block6 = block0 + 12 minutes;
    ...
    function DDDico( address _wallet, address _token , uint256 _hardCap, uint256 _tokenSale ) internal {
        ...
        tokenSale = mul(_tokenSale,pow(10,18));
        tokenLeft = tokenSale;
    }
    ...
    function validPurchase() internal constant returns (bool) {
        bool withinPeriodPurchase = withinPeriod();
        bool nonZeroPurchase = msg.value != 0;
        bool cap = weiRaised <= hardCap;
        return withinPeriodPurchase && nonZeroPurchase && cap;
    }

    // @notice Validates the time in the period for purchasing
    function withinPeriod() internal constant returns (bool){
        bool withinStageOne = block.number >= block0 && block.number <=block1;
        bool withinStageTwo = block.number >= block2 && block.number <=block3;
        bool withinStageThree = block.number >= block4 && block.number <=block5;
        return withinStageOne && withinStageTwo && withinStageThree;
    }

    // @notice Calculates the rate based on slabs
    function fetchRate() internal returns (uint256){
        if( block0 <= block.number && block2 > block.number ){
            applicableRate = 50;
            return applicableRate;
        }
        if ( block2 <= block.number && block4 > block.number ){
            applicableRate = 25;
            return applicableRate;
        }
